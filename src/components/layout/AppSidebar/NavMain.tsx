import { Link, useLocation } from "react-router-dom"; // Thay đổi import routing
import { ChevronRight } from "lucide-react";

// Giả định các UI components này bạn vẫn giữ nguyên cấu trúc import
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
export function NavMain({ items }: NavMainProps) {
  // Thay thế usePathname bằng useLocation
            ...
            <SidebarMenu>
              {group.items.map((item) => {
                if (state === "collapsed" && !isMobile) {
                  // Nếu không có subItems, render button như link
                  if (!item.subItems) {
                    return (
                      <SidebarMenuItem key={item.title}>
                        <SidebarMenuButton
